...
Riaz.addCommand({ pattern: 'insta ?(.*)', fromMe: true, desc: "Downloads from instagaram", dontAddCommandList: true }, async (message, match) => { 
    ...
    //lyfe00011-farhan_dqz
        ...
        Riaz.addCommand({ pattern: 'insta ?(.*)', fromMe: false, desc: "Downloads from instagaram", dontAddCommandList: true }, async (message, match) => { 
            const { status, type, data } = await instagram(match[1], 'julie')
            if (!status) return await message.sendMessage('not found')
            await message.client.sendMessage(message.jid, LOAD_ING, MessageType.text, { quoted: message.data });
            if (type === 'image') return await message.sendMessage(data, MessageType.image, { caption: Config.AFN , quoted: message.data })
