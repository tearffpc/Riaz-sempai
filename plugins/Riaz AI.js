...
re-coded by nexusNw and its modified by afnanplk
to bring new ai Pinky 
*/
...
if (conf.LANG == 'EN') vtalk_dsc = 'Starts to Riaz voice chat.', reply_eva = '*Reply to Any Voice Message!*'
...
Riaz.addCommand({on: 'text', fromMe: wk, dontAddCommandList: true, deleteCommand: false}, (async (message, match) => {
    if (message.message.startsWith('Riaz') && conf.TALKING_RIAZ !== 'true') {        
        ...
        var finm = message.message.replace('Riaz', '').replace(' ', '')   
...
Riaz.addCommand({on: 'text', fromMe: false, deleteCommand: false}, (async (message, match) => {
        if (conf.TALKING_RIAZ == 'true' && ((!message.jid.includes('g.us')) || (message.jid.includes('g.us') && 
...
Riaz.addCommand({ pattern: 'vtalk$', desc: vtalk_dsc,dontAddCommandList: true, fromMe: wk }, (async (message, match) => {
...
var succ_off = ''
if (conf.LANG == 'TR') {
    fulleva_dsc = 'Tam fonksiyonel Riaz özelliklerini aktif eder. Hesabınızı bir chatbota dönüştürün!'
    already_on = 'Riaz yapay zekası halihazırda tüm fonksiyonları etkin.'
    already_off = 'Riaz yapay zekası halihazırda yarı fonksiyonel çalışıyor.'
    succ_on = 'Riaz, Tam Fonksiyonel Olarak Açıldı! Lütfen Biraz Bekleyin! ✅'
    succ_off = 'Riaz, Yarı Fonksiyonel Olarak Ayarlandı! Lütfen Biraz Bekleyin! ☑️'
}
if (conf.LANG == 'EN') {
    fulleva_dsc = 'Activates full functional Riaz features. Turn your account into a ai chatbot!'
    already_on = 'Riaz artificial intelligence is already fully functional.'
    already_off = 'Riaz artificial intelligence is currently running semi-functional.'
    succ_on = 'Riaz Opened Fully Functionally! Please wait a bit! ✅'
    succ_off = 'Riaz Set to Semi-Functional! Please wait a bit! ☑️'
}
if (conf.LANG == 'ML') {
    fulleva_dsc = 'പൂർണ്ണമായും പ്രവർത്തനക്ഷമമായ Riaz സവിശേഷതകൾ സജീവമാക്കുന്നു. നിങ്ങളുടെ അക്കൗണ്ട് ഒരു ചാറ്റ്ബോട്ടാക്കി മാറ്റുക!'
    already_on = 'Riaz കൃത്രിമബുദ്ധി ഇതിനകം പൂർണ്ണമായി പ്രവർത്തിക്കുന്നു.'
    already_off = 'Riaz AI നിലവിൽ സെമി-ഫംഗ്ഷണൽ ആണ്.'
    succ_on = 'Riaz പൂർണ്ണമായും പ്രവർത്തനക്ഷമമായി തുറന്നു! കുറച്ച് കാത്തിരിക്കൂ! ✅'
    succ_off = 'സെമി-ഫങ്ഷണൽ ആയി Riaz സജ്ജമാക്കുക! കുറച്ച് കാത്തിരിക്കൂ! ☑️'
}

Riaz.addCommand({ pattern: 'sempai ?(.*)', desc: fulleva_dsc, fromMe: true,dontAddCommandList: true, usage: '.Riaz on / off' }, (async (message, match) => {
    var Riaz_status = `${conf.TALKING_RIAZ}`
    if (match[1] == 'on') {
        if (Riaz_status == 'true') {
