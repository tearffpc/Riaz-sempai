...
if (Config.WORKTYPE == 'private') {
    ...
    Riaz.addCommand({pattern: 'meme ?(.*)', fromMe: true, desc: Lang.MEMES_DESC}, (async (message, match) => {   
    ...
    Riaz.addCommand({pattern: 'meme ?(.*)', fromMe: false, desc: Lang.MEMES_DESC}, (async (message, match) => {    
    ...
    //created by afnanplk
	    ...
	    memeMaker({
            ...
            if(err) throw new Error(err)
            await message.client.sendMessage(message.jid, fs.readFileSync('MyPnky-meme.png'), MessageType.image, {filename: 'Riazmeme.png', mimetype: Mimetype.png, caption: Config.AFN});
            await info.delete();    
        });
    }		    
    }));
